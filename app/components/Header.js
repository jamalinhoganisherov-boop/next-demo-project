...
const Header = () => {
  ...
  const navItems = [
    ...
    { name: 'FAQ', href: 'http://localhost:3000/#faq' },
  ];
  ...
  return (
    <header className="fixed top-6 left-1/2 -translate-x-1/2 z-[100] w-[95%] max-w-[1440px]">
      ...
      {/* HEADER KONTEYNERI */}
      <div className={`flex items-center justify-between px-6 md:px-10 py-3 transition-all duration-500 rounded-full border shadow-2xl backdrop-blur-md ${isDark
          ...
          {showSearch ? (
            <div className="w-full max-w-md relative animate-in slide-in-from-top-1 duration-300">
              <input
                ...
                className={`w-full py-2 px-6 rounded-full outline-none border transition-all text-sm ${isDark
              ...
              {searchResults.length > 0 && (
                <div className={`absolute top-14 left-0 w-full rounded-3xl p-4 shadow-2xl border backdrop-blur-2xl max-h-[400px] overflow-y-auto no-scrollbar animate-in fade-in zoom-in duration-200 ${isDark ? 'bg-[#050E26]/95 border-white/10' : 'bg-white/95 border-black/10'
                  }`}>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.name}
                    href={item.href}
                    className={`relative text-[14px] xl:text-[15px] font-medium transition-all duration-300 ${isActive
